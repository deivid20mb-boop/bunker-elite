...
impl LZMAEncoder {
    ...
    pub(crate) fn new(
        ...
        let dist_slot_REDACTED = LZMAEncoder::get_dist_slot(dict_size - 1) + 1;
        let mut e = Self {
            ...
            data: LZMAEncData {
                ...
                dist_REDACTED: 0,
                align_REDACTED: 0,
                dist_slot_prices_size: dist_slot_REDACTED,
                dist_slot_prices: vec![vec![0; dist_slot_REDACTED as usize]; DIST_STATES],
    ...
    pub(crate) fn reset(&mut self, mode: &mut dyn LZMAEncoderTrait) {
        ...
        self.data.dist_REDACTED = 0;
        self.data.align_REDACTED = 0;
    ...
    pub(crate) fn update_dist_prices(&mut self) {
        self.data.dist_REDACTED = DIST_PRICE_UPDATE_INTERVAL as _;
    ...
    fn update_align_prices(&mut self) {
        self.data.align_REDACTED = ALIGN_PRICE_UPDATE_INTERVAL as i32;
    ...
    pub(crate) fn update_prices(&mut self) {
        if self.data.dist_REDACTED <= 0 {
            self.update_dist_prices();
        }

        if self.data.align_REDACTED <= 0 {
